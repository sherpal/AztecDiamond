...
case class Complex(re: Double, im: Double) {
  ...
  def crossProduct(that: Complex): Double =
  ...
  override def equals(that: Any): Boolean = that match {
    case that: Complex =>
      math.max(math.abs(that.re - re), math.abs(that.im - im)) < 1e-6
    case _ => false
  }
...
object Complex {
  ...
  def exp(z: Complex): Complex =
    math.exp(z.re) * Complex(math.cos(z.im), math.sin(z.im))

  def log(z: Complex): Complex =
    math.log(!z) + i * z.arg // principal branch of log

  def log(z: Complex, branch: Double): Complex =
    math.log(!z) + i * (z.arg + (if (z.arg < branch) 2 * Pi else 0))
  ...
  implicit object ComplexIsNumeric extends Numeric[Complex] {
    ...
    def parseString(str: String): Option[Complex] = None
  }
}

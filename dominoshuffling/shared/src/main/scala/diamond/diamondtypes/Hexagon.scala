...
case object Hexagon extends DiamondType {
  ...
  def transformArguments(args: Seq[Double]): Either[WrongParameterException, (Int, Int, Int)] = {
    val a = args(0).toInt
    val b = args(1).toInt
    val c = args(2).toInt
    ...
    for {
      _ <- Either.cond(
        args.forall(isInteger) && a > 0 && b > 0 && c > 0,
        (),
  ...
  /** Computes the WeightTrait for generating random lozenge tilings of the hexagon.
    ...
    * the hexagon. For example, the sub graph of a hexagon with each side 1 is ._._. \| | ._._. The sub graph for a
    * hexagon with each side 2 is ._._. \| | ._._._._. \| | | ._._._._. \| | ._._. (each weight on the existing edges is
    * then 1.0)
    *
    ...
    *
    * @param args
    *   ._1: (a) size of the top right and bottom left sides of the hexagon ._2: (b) size of the top and bottom sides of
    *   the hexagon (those parallel to the horizontal axis) ._3: (c) size of the top left and bottom right sides of the
    *   hexagon
    * @return
    *   the CustomGenerationWeight needed to generate a random tiling.
    */
  ...
  def countingTilingDiamond(args: (Int, Int, Int)): Diamond = {
    ...
    Face
      ...
      .filterNot(domino =>
        diamondConstruction.isPointOccupied(domino.p1) || diamondConstruction.isPointOccupied(domino.p2)
      )
      .foreach(diamondConstruction.update)
  ...
  val argumentNames: List[DiamondType.ArgumentName] =
    List(
      DiamondType.ArgumentName("First side size", 15, 3),
      DiamondType.ArgumentName("Second side size", 15, 3),
      DiamondType.ArgumentName("Third side size", 15, 3)
    )
}

...
trait WeightTrait[WeightType] {
   ...
   * probability distribution induced by the subWeights, then applying the algorithm gives a diamond generated according
   * to this weight.
   */
...
//  }
   ...
   */
  def inBoundsPoint(point: Point): Boolean = math.abs(point.x - 0.5) + math.abs(point.y - 0.5) <= n

  /**
   * Returns whether the domino recovers an edge of a diamond of order n.
   */
  def inBoundsDomino(domino: Domino): Boolean = inBoundsPoint(domino.p1) && inBoundsPoint(domino.p2)
 ...
 */
  def injectSubGraphDouble(n: Int, isInSubGraph: (Point) => Boolean): CustomGenerationWeight = {
  ...
  def injectSubGraphQRoot(n: Int, isInSubGraph: (Point) => Boolean): CustomComputePartitionFunctionWeight = {
...
//  }
   ...
   */
  def injectSubGraphDouble(n: Int, pointsInSubGraph: Traversable[Point]): CustomGenerationWeight = {
    val pointsSet = pointsInSubGraph.toSet
    injectSubGraphDouble(n, pointsSet.contains(_: Point))
  }

  def injectSubGraphQRoot(n: Int, pointsInSubGraph: Traversable[Point]): CustomComputePartitionFunctionWeight = {
    val pointsSet = pointsInSubGraph.toSet
...
//  }
   ...
   */
  def injectSubGraphDouble(n: Int, subGraphWeights: Map[Domino, Double]): CustomGenerationWeight = {
  ...
  def injectSubGraphQRoot(n: Int, subGraphWeights: Map[Domino, Double]): CustomComputePartitionFunctionWeight = {
    val pointsInGraph: TreeSet[Point] =
      subGraphWeights
        .keys
        .flatMap(d => Seq(d.p1, d.p2))
...
//  }
   ...
   */
  def hexagonWeightGeneration(a: Int, b: Int, c: Int): CustomGenerationWeight = {
    val order = 2 * a - 1 + b
  ...
  def hexagonWeightGeneration(n: Int): CustomGenerationWeight = hexagonWeightGeneration(n, n, n)

  def hexagonWeightPartition(a: Int, b: Int, c: Int): CustomComputePartitionFunctionWeight = {
    val order = 2 * a - 1 + b
  ...
  def rectangleOrder(width: Int, height: Int): Int = (width + height - 2 + 1) / 2 // +1 needed in case either
                                                                                  // m or n in odd

  def rectangleWeightsGeneration(width: Int, height: Int): CustomGenerationWeight = {
    injectSubGraphDouble(rectangleOrder(width, height), (point: Point) => isInRectangle(point, width, height))
  }

  def rectangleWeightsPartition(width: Int, height: Int): CustomComputePartitionFunctionWeight = {
...
//  }
  ...
  def aztecHouseWeightsGeneration(width: Int, height: Int): CustomGenerationWeight =
    injectSubGraphDouble((width + height - 2 + 1) / 2, (point: Point) => isInAztecHouse(point, width, height))
...
//  }
  ...
  def isInDoubleAztecDiamond(point: Point, order: Int, overlap: Int): Boolean = {
    val n = order
    val l = order - overlap
   ...
   */
  def doubleAztecDiamondGeneration(order: Int, overlap: Int): CustomGenerationWeight ={
  ...
  def doubleAztecDiamondPartition(order: Int, overlap: Int): CustomComputePartitionFunctionWeight ={
    val totalOrder = 4 * order - 2 * overlap + 1
...
//  }
   ...
   */
  def diamondRingGeneration(innerOrder: Int, outerOrder: Int): CustomGenerationWeight = {
    if (outerOrder > innerOrder && ((outerOrder - innerOrder) % 2 == 0 || outerOrder >= 3 * innerOrder - 1)) {
      injectSubGraphDouble(outerOrder, (point: Point) => isInDiamondRing(point, innerOrder, outerOrder))
    } else {
      throw new NotTileableException
    }
  }

  def diamondRingPartition(innerOrder: Int, outerOrder: Int): CustomComputePartitionFunctionWeight = {
    if (outerOrder > innerOrder && ((outerOrder - innerOrder) % 2 == 0 || outerOrder >= 3 * innerOrder - 1)) {
      injectSubGraphQRoot(outerOrder, (point: Point) => isInDiamondRing(point, innerOrder, outerOrder))
    } else {
      throw new NotTileableException
    }
  }
...
//
   ...
   */
  def twoPeriodicAztecDiamondPartition(a: QRoot, b: QRoot, order: Int): CustomComputePartitionFunctionWeight = {
...
//  }
   ...
   * The head of the list is the WeightMap for diamond of order 1, and the last is the argument weight.
   *
   ...
   */
  def computeAllWeights[WeightType, Weight <: WeightTrait[WeightType]](weight: Weight): List[Weight] = {
    def computeAllWeightsAcc(computed: List[Weight]): List[Weight] = {
      if (computed.head.n == 1)
        computed
      else
        computeAllWeightsAcc(computed.head.subWeights.asInstanceOf[Weight] +: computed)
    }

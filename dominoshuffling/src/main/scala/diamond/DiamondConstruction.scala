...
class DiamondConstruction(val order: Int) {
  ...
  private val dominoes: Array[Array[Option[Domino]]] = Diamond.emptyArrayDominoes(order)
  ...
  def forcedDominoes: List[Domino] =
    DiamondConstruction.allPoints(order)
  ...
  def insertDiamond(diamond: Diamond, center: Point = Point(0, 0)): Unit = {
    diamond.listOfDominoes.map({ case Domino(p1, p2) => Domino(p1 + center, p2 + center)}).foreach(this() = _)
  }
...
object DiamondConstruction {
  ...
  def allPoints(order: Int): Traversable[Point] = (for {
    y <- 1 to order
    x <- -order + y to order + 1 - y
  } yield Point(x, y)).flatMap({ case Point(x, y) => List(Point(x, y), Point(x, -y + 1))})

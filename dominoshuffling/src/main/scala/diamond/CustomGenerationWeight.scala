...
object CustomGenerationWeight {
  ...
  /**
   * Computes a GenerationWeight associated with the picture whose information is contained within imageData.
   * @param imageData width * height * 4 Vector containing the integers of a canvas imageData
   * @param width     width of the canvas
   * @param height    height of the canvas
   * @return
   */
  def fromImageData(imageData: Vector[Int], width: Int, height: Int): CustomGenerationWeight = {
    val order = Rectangle.diamondOrder(width, height)

    val weights = new CustomGenerationWeight(Rectangle.diamondOrder(width, height))

    val isPointInRectangle: (Point) => Boolean = Rectangle.isPointInDiamond(width, height)

    val isDominoInRectangle: (Domino) => Boolean = Rectangle.isInDiamond(width, height)

    val (facesIn, facesOut) = Face.activeFaces(order).partition(_.dominoes.forall(isDominoInRectangle))

    facesOut.flatMap(_.dominoes).foreach(domino => {
      if (isPointInRectangle(domino.p1) && !isPointInRectangle(domino.p2) ||
    ...
    val greyScale = imageData.grouped(4).map(_.take(3).sum / 3).map(_ / 255.0).map(clamp).toVector
    ...
    facesIn.foreach(face => {
      ...
      face.dominoes.foreach(domino => weights(domino) = if (domino.isHorizontal) luminosity else 1 - luminosity)
    })

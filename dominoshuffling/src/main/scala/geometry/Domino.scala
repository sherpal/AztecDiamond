...
object Domino {
  private def yMin(x: Int, n: Int): Int = if (x > 0) -n + x else -n + 1 - x

  private def xMin(y: Int, n: Int): Int = if (y > 0) -n + y else -n + 1 - y

  def changeHorizontalCoordinates(point: Point, n: Int): (Int, Int) =
    (point.x - xMin(point.y, n), point.y + n - 1)

  def changeVerticalCoordinates(point: Point, n: Int): (Int, Int) =

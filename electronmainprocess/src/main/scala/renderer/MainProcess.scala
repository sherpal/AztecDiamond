...
object MainProcess {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def createWindow(): Unit = {
      ...
      win.loadURL(
        "file://" + Path.join(
          Path.dirname(ElectronGlobals.__dirname),
          "/mainwindow/html/domino-shuffling-implementation.html"
        )
      )
      ...
      win.webContents.on("did-finish-load", () => Storage.storeVariable(win.webContents, "windowId", WindowId(win.id)))

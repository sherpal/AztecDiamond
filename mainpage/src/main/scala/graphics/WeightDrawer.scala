...
class WeightDrawer(weights: WeightTrait[QRoot]) {
  ...
  private val points: GenSeq[PointSprite] = Face.activeFaces(weights.n)
    .flatMap(_.dominoes)
    .flatMap(d => List(d.p1, d.p2))
    .map(p => PointSprite(p.x, p.y))


  private val dominoes: GenSeq[WeightSprite] = Face.activeFaces(weights.n)
    .flatMap(_.dominoes)
    .map(d => WeightSprite(d, weights(d)))

  private val activeFaces: GenSeq[ActiveFaceSprite] = Face.activeFaces(weights.n).map(ActiveFaceSprite)
  private val activeFacesColor: Vec4 = if (weights.n % 2 == 0) Vec4(1,0,0,0.2) else Vec4(0,1,0,0.2)

  activeFaces.foreach(_.setColor(activeFacesColor))

  def draw(worldCenter: Complex = center, scaleX: Double = 1, scaleY: Double = 1): Unit = {
    ...
    def draw(canvas2D: Canvas2D, x: Double, y: Double, width: Double, height: Double): Unit = {
      ...
      weight match {
        case weight: Rational =>
          val numWidth = canvas2D.textWidth(weight.numerator.toString, canvas2D.font)
          val denWidth = canvas2D.textWidth(weight.denominator.toString, canvas2D.font)

          canvas2D.print(
            List((weight.numerator.toString, Vec4(0,0,0,1).toCSSColor)),
            center - numWidth / 2, width, height,
            font = "10px monospace"
          )
          ...
          if (weight.denominator != 1 && weight.numerator != 0) {
            canvas2D.print(
              ...
              center - denWidth / 2 + 12 * Complex.i, width, height,
              font = "10px monospace"
            )
            ...
            canvas2D.drawLine(
              List(center - lineLength / 2 + 6 * Complex.i, center + lineLength / 2 + 6 * Complex.i),
              Vec3(0,0,0), lineWidth = 1
            )
          }
        case _: NotRational =>

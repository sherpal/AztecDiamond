...
//  private val backupNumber: Int = math.max(1, order / 5)
  ...
  private val backupDiamonds: Map[Int, Diamond] = {
    (diamond +: (order to 2 by -1).foldLeft((diamond, List[Diamond]()))({
      case ((previousDiamond, backups), _) =>
        val newDiamond = previousDiamond.randomSubDiamond.get
        (newDiamond, if (backupOrders.contains(newDiamond.order)) newDiamond +: backups else backups)
    })._2).map(d => (d.order, d)).toMap
  }


  if (scala.scalajs.LinkingInfo.developmentMode) {
    println(s"It took ${new java.util.Date().getTime - startInitializationTime}ms to compute the " +
      s"${backupDiamonds.size} backups.")
  }

  private def computeDiamond(diamondOrder: Int): Diamond = {
    val startingDiamond = backupDiamonds(backupOrders.find(_ >= math.max(1, math.min(order, diamondOrder))).get)

    (1 to startingDiamond.order - diamondOrder)
      .foldLeft(startingDiamond)((prevDiamond, _) => prevDiamond.randomSubDiamond.get)
  }

  private var _currentDrawer: DiamondDrawer = DiamondDrawer(computeDiamond(1)).get
  ...
  def changeDrawer(diamondOrder: Int): DiamondDrawer = {
    ...
    if (_currentDrawer.diamond.order != actualOrder) {
      ...
      _currentDrawer = DiamondDrawer(computeDiamond(actualOrder), canvas = Some((canvas, ctx))).get
    }

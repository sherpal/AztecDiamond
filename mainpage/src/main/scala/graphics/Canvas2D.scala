...
class Canvas2D(val canvas: html.Canvas, ctx: CanvasRenderingContext2D) {
   ...
   * /!\ This automatically binds the context to the canvas, so it can't be done after creation.
   */
  ...
  def drawVertices(vertices: Seq[Complex], color: Vec4, lineWidth: Int = 0, cycle: Boolean = false): Unit =
  ...
  def printWatermark(scaleX: Double, scaleY: Double): Unit = {
    val name = List(65, 110, 116, 111, 105, 110, 101, 32, 68, 111, 101, 114, 97, 101, 110, 101)
      ...
      .mkString
    val info = List(
      104, 116, 116, 112, 115, 58, 47, 47, 115, 105, 116, 101,
      115, 46, 117, 99, 108, 111, 117, 118, 97, 105, 110, 46,
      98, 101, 47, 97, 122, 116, 101, 99, 100, 105, 97, 109,
      111, 110, 100, 47
    )
      .map(_.toChar)
      .mkString
...
//      .mkString
    ...
    print(
      List((name, "#CCC")),
      Complex(width - textWidth(name, font) - 10, height - textHeight), maxTextWidth, textHeight / 2, font = font
    )

    print(
      List((info, "#CCC")),
      Complex(width - textWidth(info, font) - 10, height - textHeight / 2), maxTextWidth, textHeight / 2, font = font
    )
  }
   ...
   * @param center The center of the rotation, in cartesian coordinate. 0 is the center of the canvas, and
   *               -w/2 + h/2 i is the top left.
   * @param matrix The (3d) transformation matrix to apply.
   * @return       The (3d) transformation matrix to give to "withTransform" method.
   */
  private def withCenter(center: Complex)(matrix: Matrix): Matrix = {
    ...
    translate(localCenter.re, localCenter.im) * matrix * translate(-localCenter.re, -localCenter.im)
  }
...
//  }
   ...
   * @param center The center of the rotation, in cartesian coordinate. 0 is the center of the canvas, and
   *               -w/2 + h/2 i is the top left.
   * @param angle  The angle of the rotation. Positive value for counter clockwise rotation.
   * @return       The (3d) transformation matrix to give to "withTransform" method.
   */
...
//  }
  ...
  def translate(dx: Double, dy: Double): Matrix = Matrix.translation3d(dx, dy, 1)
  ...
  private def setTransform(): Unit = {
    ctx.setTransform(
      _transformationMatrix(0,0), _transformationMatrix(1,0),
      _transformationMatrix(0,1), _transformationMatrix(1,1),
  ...
  def withTransformationMatrix[A](matrix: Matrix)(body: => A): A = {
    ...
    setTransform()
    try body finally resetTransformationMatrix()
  }

...
class DominoSprite(val domino: Domino) extends Sprite {
  ...
  setWorldSize(if (domino.isHorizontal) 2 else 1, if (domino.isHorizontal) 1 else 2)

  if domino.isHorizontal then setWorldPos(Complex(domino.p1.x + 1, domino.p1.y + 0.5))
  else setWorldPos(Complex(domino.p1.x + 0.5, domino.p1.y + 1))

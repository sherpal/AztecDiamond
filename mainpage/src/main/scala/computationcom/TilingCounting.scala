...
trait TilingCounting extends Computer {
  ...
  protected def receiveMessage(message: Message): Unit = {
    ...
    message match {
      ...
      case TilingMessage(diamondTypeString, time, args, weightInfo, probabilityInfo) =>
        ...
        if (scala.scalajs.LinkingInfo.developmentMode) {
          println(s"It took $time ms to compute the number of tilings (args: $args).")
        }
...
object TilingCounting {
  ...
  val startCountingButton: html.Input = dom.document.getElementById("computePartitionStart").asInstanceOf[html.Input]
  val cancelButton: html.Input = dom.document.getElementById("computePartitionCancel").asInstanceOf[html.Input]
  cancelButton.disabled = true

  val computePartitionInfo: html.Paragraph =
    dom.document.getElementById("computePartitionInfo").asInstanceOf[html.Paragraph]
  ...
  private[computationcom] def compute(computer: (Message) => TilingCounting): Unit = {
    CountingTilingForm.args match {
      case Some(arguments) =>
        startCountingButton.disabled = true
        cancelButton.disabled = false

        workingComputer = Some(computer(
